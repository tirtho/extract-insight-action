...
if [ -z "${JAVA_HOME:-}" ]; then
    echo "[ERROR] JAVA_HOME is not set. Please set JAVA_HOME to a JDK 21 installation."
    exit 1
fi
JAVA_MAJOR_VERSION=$("$JAVA_HOME/bin/java" -version 2>&1 | head -1 | grep -oP '"\K\d+')
if [ -z "$JAVA_MAJOR_VERSION" ]; then
    echo "[ERROR] Could not detect Java version from JAVA_HOME ($JAVA_HOME)"
    exit 1
fi
if [ "$JAVA_MAJOR_VERSION" != "21" ]; then
    echo "[ERROR] This project requires Java 21 but JAVA_HOME points to Java $JAVA_MAJOR_VERSION."
    echo "        JAVA_HOME: $JAVA_HOME"
    echo "        Please install JDK 21 and update JAVA_HOME."
    exit 1
fi
...
if ! set_function_app_settings "$FUNC_MAILBOX_NAME" "$RESOURCE_GROUP_NAME" \
    ...
    "AZURE_KEY_VAULT_URL=$KV_URL"; then
