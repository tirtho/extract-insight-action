...
class AzGetConnectionIT {

    private static AzGetConnection connection;
    ...
    TestWatcher testResultLogger = new TestWatcher() {
        ...
        public void testSuccessful(ExtensionContext ctx) {
            System.out.println("<<< END  : Test #" + getOrder(ctx) + " - " + ctx.getDisplayName() + " - PASSED");
        }

        @Override
        public void testFailed(ExtensionContext ctx, Throwable cause) {
            System.out.println("<<< END  : Test #" + getOrder(ctx) + " - " + ctx.getDisplayName() + " - FAILED");
        }

        @Override
        public void testAborted(ExtensionContext ctx, Throwable cause) {
            System.out.println("<<< END  : Test #" + getOrder(ctx) + " - " + ctx.getDisplayName() + " - ABORTED");
        }
    };
    ...
    void logTestStart(TestInfo testInfo) {
        int order = testInfo.getTestMethod()
                ...
                .map(Order::value)
                .orElse(-1);
        System.out.println(">>> START: Test #" + order + " - " + testInfo.getDisplayName());
    }
    ...
    static void initConnection() {
        ...
        connection = new AzGetConnection(kvUrl);
    }
    ...
    void cosmosClient_connects() {
        ...
        String cosmosdbContainerIdString = connection.getCosmosContainer().getId(); // just to prove KV reads work
    ...
    void cosmosContainer_isAccessible() {
        ...
        System.out.println("  ✓ Cosmos container: " + props.getId());
    }
    ...
    void serviceBusSender_connects() {
        ...
        System.out.println("  ✓ Service Bus sender namespace: " + ns);
    ...
    void serviceBusReceiver_connects() {
        ...
        assertNotNull(ns, "Fully qualified namespace should not be null");
        assertFalse(ns.isBlank(), "Namespace should not be blank");
        System.out.println("  ✓ Service Bus receiver namespace: " + ns);
    ...
    void aiFoundryChatClient_responds() {
        ...
        System.out.println("  ✓ AI Foundry model reply: " + reply.trim());
    }
    ...
    void contentUnderstanding_endpointIsValid() {
        ...
        System.out.println("  ✓ Content Understanding endpoint: " + endpoint);
    ...
    @Test
    @Order(7)
    @DisplayName("Content Understanding – list analyzers returns at least one")
    void contentUnderstanding_listAnalyzers() throws Exception {
        String endpoint = connection.getContentUnderstandingEndpoint();
        DefaultAzureCredential credential = connection.getContentUnderstandingCredential();
        ...
        // Call the Content Understanding REST API to list analyzers
        String url = endpoint + "/contentunderstanding/analyzers?api-version=2024-12-01-preview";
        ...
        // Skip if the Content Understanding preview API is not available in this region
        assumeTrue(response.statusCode() != 404,
                "Content Understanding preview API is not supported in this region — skipping. "
                        + "Response: " + response.body());

        assertEquals(200, response.statusCode(),
                "Expected 200 OK but got " + response.statusCode() + ": " + response.body());
        ...
        // Ensure the array is not empty – at least one analyzer must exist
        assertFalse(body.contains("\"value\":[]")
                        && !body.contains("\"value\": []"),
                "Expected at least one analyzer in the list");
        System.out.println("  ✓ Content Understanding analyzers response length: " + body.length());
    }
    ...
    void graphClient_readsMailboxInbox() {
        ...
        System.out.println("  ✓ Mailbox folder: " + mailboxFolder);
        ...
        System.out.println("  ✓ Inbox display name : " + inbox.getDisplayName());
        System.out.println("  ✓ Total item count   : " + inbox.getTotalItemCount());
        System.out.println("  ✓ Unread item count  : " + inbox.getUnreadItemCount());
    }
    ...
    void keyVaultAccessors_returnValues() {
        ...
        System.out.println("  ✓ AI Foundry deployment : " + connection.getAiFoundryDeploymentName());
        System.out.println("  ✓ AI Foundry model      : " + connection.getAiFoundryModelName());
        System.out.println("  ✓ AI Foundry API version: " + connection.getAiFoundryApiVersion());
        System.out.println("  ✓ CU endpoint           : " + connection.getContentUnderstandingEndpoint());
    }
}
