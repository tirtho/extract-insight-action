...
//@Disabled("Temporarily disabled to isolate JVM hang issue")
class AzConnectionTest {
    ...
    @BeforeEach
    void setUp() {
        connection = new AzConnection(secretClient);
    }
    ...
    @Test
    void getAiFoundryChatClient_cachesClientOnSubsequentCalls() {
        stubSecret(AzEnvNames.KV_AI_FOUNDRY_ENDPOINT, "https://my-foundry.openai.azure.com");

        OpenAIClient first = connection.getAiFoundryChatClient();
        OpenAIClient second = connection.getAiFoundryChatClient();

        assertSame(first, second, "getAiFoundryChatClient should return the cached instance");
        verify(secretClient, times(1)).getSecret(AzEnvNames.KV_AI_FOUNDRY_ENDPOINT);
    }

    // ---------------------------------------------------------------
    //  Content Understanding HttpClient – verify caching
    // ---------------------------------------------------------------

    @Test
    void getContentUnderstandingHttpClient_cachesClientOnSubsequentCalls() {
        java.net.http.HttpClient first = connection.getContentUnderstandingHttpClient();
        java.net.http.HttpClient second = connection.getContentUnderstandingHttpClient();

        assertNotNull(first);
        assertSame(first, second, "getContentUnderstandingHttpClient should return the cached instance");
    }
